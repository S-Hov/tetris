...
Документация описывает, как работать с клиентской частью PVP Blocks. Это не учебник React с нуля, а практическое соглашение по этому проекту: куда класть код, как думать о компонентах, как использовать hooks, как оформлять формы и секции.
...
- `features` держат конкретный сценарий: auth form, game flow, email verification.
