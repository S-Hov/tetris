...
- `friendships` хранит заявки и подтвержденные связи между пользователями
- `user_privacy_settings` хранит режимы `public`, `friends`, `private` для профиля, заявок, приглашений и сообщений

### `user_privacy_settings`

Настройки приватности связаны с `users` отношением 1:1 через `user_id`. Для новых пользователей
строка создается trigger-функцией. Все поля видимости принимают только `public`, `friends` или
`private`; заявки в режиме `friends` разрешены пользователям с общим другом.
