...
Каталог эффектов в БД (`game_effects`) является источником истины для ключей,
активности, длительности, текстов и параметров баланса. Исполняемые реализации
хранятся отдельно в клиентском реестре эффектов. Каталог доступен через админку.
