...
export const verifyEmailSchema = z.object({
    code: z.string().trim().regex(/^\d+$/, 'Код должен состоять только из цифр'),
})
...
export const changeUnverifiedEmailSchema = z.object({
    currentEmail: z.string().email('Неверный формат текущей почты'),
    email: z.string().email('Неверный формат новой почты'),
})
