...
const PcControlsPage = () => {
    ...
    const handleSave = () => {
        ...
        if (emptyAction) {
            notify(`Назначьте клавишу для действия "${PC_CONTROL_ACTION_LABELS[emptyAction]}"`, 'warning')
            return
        }
        ...
        changedActions.forEach((action) => {
            notify(
                `${PC_CONTROL_ACTION_LABELS[action]} назначено на ${formatKeyCode(nextSettings.bindings[action][0])}`,
                'success'
            )
        })
    }
            ...
            <div className="container pc-controls-page__container">
                <GlowEffect>
                    <header className="pc-controls-page__topbar">
                        <button className="pc-controls-page__back" type="button" onClick={() => navigate(-1)} aria-label="Назад">
                            <i className="fas fa-arrow-left"></i>
                        </button>
                        <div>
                            <p className="pc-controls-page__eyebrow">Настройки управления</p>
                            <h1>Управление для ПК</h1>
                        </div>
                    </header>
                </GlowEffect>
                <GlowEffect>
                        ...
                        <div className="pc-controls-page__list">
                            {PC_CONTROL_ACTION_ORDER.map((action) => (
                                <div className="pc-controls-page__row" key={action}>
                                    <span>{PC_CONTROL_ACTION_LABELS[action]}</span>
                                    <button
                                        type="button"
                                    ...
                                    >
                                        {listeningAction === action
                                            ? 'Нажмите клавишу'
                                            : (draftBindings[action] || []).map(formatKeyCode).join(' / ') || 'Не назначено'}
                                    </button>
                                </div>
                            ))}
                        </div>
